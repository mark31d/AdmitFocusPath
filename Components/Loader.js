// Components/Loader.js — AdmiralFocus Splash / Loading (Uiverse warp loader via WebView)
...
/* ── assets ── */
const IMG_BG = require('../assets/bg.png');
...
/* ── main loader ── */
function Loader({ fullscreen = false, color = THEME.gold }) {
  const size = 250;
  ...
  const injectedJavaScript = `
    (function() {
      ...
      document.addEventListener('DOMContentLoaded', function() {
        const svg = document.querySelector('.pl');
        if (svg) {
      ...
      // Убеждаемся, что анимации запущены
      setTimeout(function() {
        const animatedElements = document.querySelectorAll('.pl__ring-rotate, .pl__ring-stroke, .pl__arrows, .pl__tick');
  ...
  const webViewProps = {
    ...
    onLoadEnd: () => {
      // Анимации должны запуститься автоматически через CSS
    },
  };
  ...
  if (!fullscreen) {
    return (
      <View style={styles.inlineWrap}>
        <View style={[styles.warpLoader, { width: size, height: size }]}>
